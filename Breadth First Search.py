...
def BFS(m):  # largeur d'abord
    ...
    bfsPath = {}  # dictionnaire bfsPath prochainement utile pour l'affichage de la route directe
    while len(route) > 0:
        ...
        # la matrice est accessible a partir de la requette maze_map[],
        ...
        for d in 'WNES':  # d: direction, WNES: les mouvements possibles
            if m.maze_map[case][d] == True:  # d true si le chemin traité est libre (sans obstacles)
                ...
                bfsPath[prochaineCase] = case  # bfsPath de la prochaine case recoit la case avant elle (pour tracer la route directe vers le but)
    ...
    while pathCase != debut:  # tq on est pas encore arrivé au debut
        path[bfsPath[pathCase]] = pathCase  # path[predecesseur de case] recoit case
        pathCase = bfsPath[pathCase]  # on avance, case recoit la case avant elle
...
path = BFS(labyrinthe)  # Itineraire a suivre dans la variable path
chercheur = agent(labyrinthe, footprints=True)  # agent en bleu
labyrinthe.tracePath({chercheur: path})  # pour que l'agent suit l'itineraire generé par la fct BFS
