...
def AEtoile(m, debut, explored, g_score, f_score, open, aPath):  # A*
        ...
        for d in 'WNES':  # d: direction, WNES: les mouvements possibles
            if m.maze_map[case][d] == True:  # d true si le chemin traité est libre (sans obstacles)
                ...
                if temp_f_score < f_score[prochaineCase]:  # si la prochaine case est moins couteuse
                    ...
                    open.put((temp_f_score, h(prochaineCase, (1, 1)), prochaineCase))  # enfiler cette prochaine case dans open
                    aPath[prochaineCase] = case  # aPath de la prochaine case recoit la case avant elle (pour tracer la route directe vers le but)
...
g_score = {case: float('inf') for case in labyrinthe.grid}  # fct g donne la distance entre le debut et la case en cours
g_score[debut] = 0
f_score = {case: float('inf') for case in labyrinthe.grid}  # fct f est le cout de chemin,
...
path = AEtoile(labyrinthe, debut, explored, g_score, f_score, open, aPath)  # Itineraire a suivre dans la variable path
